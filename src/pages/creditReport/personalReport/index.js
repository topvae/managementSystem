/*
 * @Author: your name
 * @Date: 2020-02-17 12:33:52
 * @LastEditTime: 2020-03-19 17:34:43
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: /credit-admin/src/pages/creditReport/personalReport/index.js
 */
import React, { useState, useEffect, useCallback } from 'react'
import { Card, Button, Modal, Icon } from 'antd'
import BaseForm from '../../../components/Form'
import { config, changeSelectParams } from './formList'
import { get_options, query_product_list, query_credit_report_list } from '../../../services/api'
...
function PersonalReport() {
  ...
  useEffect(() => {
    async function fetchData() {
      const res = await get_options({ 'businessIdent': 't_credit_subject_person', 'field': 'person_id_number_type' })
      ...
      if (data) {
        ...
        const personalReport = JSON.parse(sessionStorage.getItem('personalReport'))
        if (!personalReport) return
        const { url, text, idNumber, creditSubjectName, idType, value } = personalReport
        if (url) {
          setUrl(host + url)
          setNoSearch(false)
        } else {
    ...
    async function getServe() {
      ...
      if (creditTypeResponseData) {
        const fieldCode = creditTypeResponseData.filter(item => item.fieldCode === '0')
  ...
  const setSession = (params, url) => {
    ...
    sessionStorage.setItem('personalReport', JSON.stringify(obj))
  }

  // 重置表格的时候
  const resetFields = () => {
    sessionStorage.removeItem('personalReport')
    setNoSearch(true)
  }
  ...
  return (
    <div className='personalReport'>
      ...
      <Card bordered={ false } className='searchResult'>
        <AuthButton type='primary' className='export' onClick={ exportReport } menu_id={ 177 }>导出报告</AuthButton>
